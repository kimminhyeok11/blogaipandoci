...
const renderer = {
  ...
  image(token: { href: string; title?: string | null; text: string }) {
    // alt 속성이 비어있으면 기본값 설정
    const altText = token.text?.trim() || "이미지";
    // aspect-ratio와 기본 크기로 CLS 방지
    return `<figure class="my-6 max-w-2xl mx-auto"><img src="${token.href}" alt="${altText}" class="w-full h-auto rounded shadow-sm" loading="lazy" decoding="async" width="800" height="450" style="aspect-ratio: 16/9;"${token.title ? ` title="${token.title}"` : ''} />${token.title ? `<figcaption class="text-center text-xs text-muted mt-2 font-sans">${token.title}</figcaption>` : ''}</figure>`;
  },
